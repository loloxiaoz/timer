...
class CronEventHandler
{
    public function __construct()
    {
        ...
        $this->logger           = XLogKit::logger("handle");
    ...
    public function logicHandle($msg)
    {
        ...
        if (!$timer) {
            // 定时任务不存在
            $this->logger->error("timer not exist, msg=" . json_encode($msg));
        ...
        if ($msg["expireTime"]) {
            ...
            if ($currentTimestamp - $expireTimestamp > 30) {
                ...
                $this->logger->info("timer process after expire " . ($currentTimestamp - $expireTimestamp) . "s. msg=" . json_encode($msg));
            }
        }

        if (!$this->notifyHandler->isSupport($timer->callback["type"])) {
            $this->logger->error("timer type not support, msg=" . json_encode($msg) . " timer=" . json_encode($timer->getPropArray()));
        ...
        if (!$deleteResult) {
            ...
            $this->logger->error("timer del timer fail, msg=" . json_encode($msg) . " timer=" . json_encode($timer->getPropArray()));
            StatDao::ins()->incrError($app,CronConstants::OTHER_ERROR);
        }

        if ($result) {
            $this->logger->info("timer notify success, msg=" . json_encode($msg) . " timer=" . json_encode($timer->getPropArray()));
            return true;
        } else {
            return false;
        }
    ...
    private function nofityRetry($timer)
    {
        ...
        if($currentCalltimes % 10 == 0){
            $this->errorLogger->error("url= 'http://".$params["host"].":".$params["port"].$params["url"]."' -H 'Host:".$params["domain"]."' already_retry_times=$currentCalltimes max_retry_times=$maxRetry timerId=" . $timer->timerId);
        }
    ...
    public function handle($msg)
    {
        ...
        try {
            ...
            if($r) {
                $this->logger->info("succ useTime $totalTime,msg=".$msg);
            } else {
                $this->logger->error("fail useTime $totalTime,msg=".$msg);
            }
            ...
            $etime     = microtime(true);
            $totalTime = sprintf("%.3f", $etime-$stime);
            $this->logger->error("fail useTime $totalTime,errmsg=".$e->getMessage().", msg=".$msg);
